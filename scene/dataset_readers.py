...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png" , depth_extension = None , seg_extension=None):
    cam_infos = []
    if not os.path.exists(os.path.join(path, transformsfile)):
        return None 
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            depth = None
            seg = None
            if depth_extension: # read depth
                depth_path = os.path.join(path, frame["file_path"] + depth_extension).replace('rgb','depth')
                # 读取npz格式     
                data = np.load(depth_path)
                # 获取深度图
                depth = data['depth']

            if seg_extension: # read depth
                seg_path = os.path.join(path, frame["file_path"] + seg_extension).replace('rgb','seg')
                # 读取npz格式     
            ...
            FovY = fovy
            FovX = fovx
            if 'proj_transform' in frame:
                proj_transform = np.array(frame["proj_transform"])
            else:
                proj_transform = None
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                        image_path=image_path, image_name=image_name, width=image.size[0],
                                        height=image.size[1], fid=frame_time , proj_transform = proj_transform , depth= depth , seg = seg, view = view))
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    if not os.path.exists(ply_path):
        ...
        # xyz = np.random.random((num_pts, 3)) * 3 - 1.5 # * 4. - 2. # * 2.6 - 1.3
...
def readPhysicsBenchmark(path, white_background, eval, extension=".jpg", depth_extension='.npz' , seg_extension = '.npz'):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(
        path, "transforms_train.json", white_background, extension, depth_extension , seg_extension)
    init_cam_infos = [cam_info for cam_info in train_cam_infos if cam_info.fid == 0.]

    print("Reading Val Transforms")
    val_cam_infos = readCamerasFromTransforms(
        path, "transforms_val.json", white_background, extension, depth_extension , seg_extension)

    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(
        path, "transforms_test.json", white_background, extension, depth_extension , seg_extension)

    # 获取 train 和 val 中所有的 view 值集合
    train_views = {cam.view for cam in train_cam_infos if cam.view is not None}
    val_views = {cam.view for cam in val_cam_infos if cam.view is not None}

    # 重新分配 test_cam_infos 到 train 或 val
    reassigned_train = []
    reassigned_val = []
    for cam in test_cam_infos:
        view = cam.view
        if view in train_views:
            reassigned_train.append(cam)
        elif view in val_views:
            reassigned_val.append(cam)

    # 合并重新分配的 cam_infos
    train_cam_infos.extend(reassigned_train)
    val_cam_infos.extend(reassigned_val)
    test_cam_infos = []
...
sceneLoadTypeCallbacks = {
    ...
    "PhysicsBenchmark": readPhysicsBenchmark,
}
