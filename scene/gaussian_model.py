...
class GaussianModel:
    ...
    def load_ply(self, path, og_number_points=-1, sample_factor=-1):
        self.og_number_points = og_number_points
        plydata = PlyData.read(path)
        ...
        xyz = np.stack((np.asarray(plydata.elements[0]["x"]),
                        ...
                        np.asarray(plydata.elements[0]["z"])), axis=1)
        # opacities = np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis]
        opacities = np.zeros_like(np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis])
        features_dc = np.zeros((xyz.shape[0], 3, 1))
        features_dc[:, 0, 0] = np.asarray(plydata.elements[0]["f_dc_0"])
        features_dc[:, 1, 0] = np.asarray(plydata.elements[0]["f_dc_1"])
        features_dc[:, 2, 0] = np.asarray(plydata.elements[0]["f_dc_2"])
        ...
        if sample_factor > 0:
            num_points = int(xyz.shape[0] * sample_factor)
            ...
            rots = rots[indices]
            deform_code = deform_code[indices]
