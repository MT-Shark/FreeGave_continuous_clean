...
class DeformModel:
    ...
    #     if time_emb[0, 0] >= dt and time_emb[0, 0] < max_time + dt:
    #         sign = 1 if torch.rand(1)[0] > 0.5 else -1
    #         d_xyz_deform, d_rotation, d_scale =  self.deform(deform_code, time_emb - dt * sign)
    #         d_xyz_vel = self.vel.integrate_pos(xyz + d_xyz_deform.detach(), time_emb - dt * sign, time_emb,
    #                                               dt) - xyz.detach() - d_xyz_deform.detach()
    #         d_xyz = d_xyz_vel + d_xyz_deform
    #     elif time_emb[0, 0] >= max_time + dt:
    #         deform_time = torch.ones_like(time_emb) * max_time
    #         d_xyz_deform, d_rotation, d_scale = self.deform(deform_code, deform_time)
    #         d_xyz_vel = self.vel.integrate_pos(xyz + d_xyz_deform.detach(), deform_time, time_emb,
    #                                               dt) - xyz.detach() - d_xyz_deform.detach()
    #         d_xyz = d_xyz_vel + d_xyz_deform
    #     else:
    ...
    # def step(self, xyz, time_emb, deform_code, dt=1/30, max_time=0.75):
    #     if time_emb[0, 0] >= dt and time_emb[0, 0] < max_time + dt:
    #         sign = 1 if torch.rand(1)[0] > 0.5 else -1
    #         d_xyz_deform, d_rotation, d_scale =  self.deform(deform_code, time_emb - dt * sign)
    #         xyz_vel, d_rotation2 = (self.vel.integrate_pos(deform_code.detach(), xyz + d_xyz_deform.detach(), time_emb - dt * sign, time_emb, dt, rot=True))
    #         d_xyz_vel = xyz_vel - xyz.detach() - d_xyz_deform.detach()
    #         d_xyz = d_xyz_vel + d_xyz_deform
    ...
    #         d_xyz_deform, d_rotation, d_scale = self.deform(deform_code, deform_time)
    #         xyz_vel, d_rotation2 = (
    #             self.vel.integrate_pos(deform_code.detach(), xyz + d_xyz_deform.detach(), deform_time, time_emb, dt, rot=True))
    #         d_xyz_vel = xyz_vel - xyz.detach() - d_xyz_deform.detach()
    #         d_xyz = d_xyz_vel + d_xyz_deform
    #         d_rotation = quaternion_multiply(d_rotation, d_rotation2)
    #     else:
    #         d_xyz, d_rotation, d_scale = self.deform(deform_code.detach(), time_emb)
    #     return d_xyz, d_rotation, d_scale

    def step_no_rot(self, xyz, time_emb, deform_code, dt=1/88, max_time=0.75):
    ...
    def step_full_rot(self, xyz, time_emb, deform_code, dt=1/88, max_time=0.75):
        max_time = self.max_time
    ...
    def step(self, xyz, time_emb, deform_code, dt=1/88, max_time=0.75):
        max_time = self.max_time
