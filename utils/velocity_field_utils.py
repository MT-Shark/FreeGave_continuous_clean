...
class WeightNet_zeropadding(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(out.shape)==2:
            zero = torch.zeros(out.shape[0],6).cuda()
            out = torch.cat([out,zero],dim=-1)
        elif len(out.shape)==1:
            zero = torch.zeros(6).cuda()
            out = torch.cat([out,zero],dim=-1)
        return out
class SegVel(nn.Module):
    def __init__(self, deform_code_dim=8, hidden_dim=64, layers=4, encode_dim=3 ,zero_padding =True):
        super(SegVel, self).__init__()
        self.K = deform_code_dim
        in_dim = 1 + 1 * 2 * encode_dim
        self.embedder = PositionEncoder(encode_dim)
        if zero_padding:    
            self.weight_net = WeightNet_zeropadding(in_dim,hidden_dim,layers,self.K)
        else:
            self.weight_net = nn.Sequential(nn.Linear(in_dim, hidden_dim), nn.SiLU())
    ...
    def get_vel_jac(self, deform_seg, xt):
        ...
        jac = torch.einsum('...k,...kmn->...mn', deform_seg, jac)
        return v, jac
