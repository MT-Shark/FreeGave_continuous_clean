...
def calculate_flow(render_t_1, render_t_2):
    ...
    predicted_flow_by_gs = (cov_multi + proj_2D_t_2[gs_per_pixel] - proj_2D_t_1[gs_per_pixel].detach() - x_mu.permute(0,2,3,1).detach()) * weight_per_gs_pixel.detach()[...,None]
    return predicted_flow_by_gs.sum(0)


def flow_loss(predicted_flow_by_gs, optical_flow , flow_thresh=0.1):
    ...
    Lflow = torch.norm((optical_flow - predicted_flow_by_gs)[large_motion_msk], p=2, dim=-1).mean() 
    return Lflow
