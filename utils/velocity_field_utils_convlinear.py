...
class SegVel(nn.Module):
    def __init__(self, deform_code_dim=8, hidden_dim=64, layers=4, encode_dim=3):
        ...
        self.weight_net = nn.Sequential(GroupLinear(in_dim, hidden_dim, self.K), nn.SiLU())
        for i in range(layers - 1):
            self.weight_net.append(nn.Sequential(GroupLinear(hidden_dim, hidden_dim, self.K), nn.SiLU()))
        self.weight_net.append(nn.Sequential(GroupLinear(hidden_dim, 6, self.K)))
    ...
    def forward(self, deform_code, xt):
        ...
        t_embed = self.embedder(xt[..., -1:]).unsqueeze(-2).expand(-1, self.K, -1)
