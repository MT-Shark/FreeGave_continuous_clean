...
def render_detach(viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, d_xyz, d_rotation, d_scaling,
           is_6dof=False,
           scaling_modifier=1.0, override_color=None):
    """
    Render the scene.
    ...
    if is_6dof:
        ...
        means3D = pc.get_xyz.detach() + d_xyz
    means2D = screenspace_points
    ...
    if pipe.compute_cov3D_python:
        ...
        if isinstance(d_scaling, float):
            ...
            scales = pc.modify_scaling(d_scaling).detach()
            # scales = pc.get_scaling + d_scaling
        # rotations = pc.get_rotation + d_rotation
        if isinstance(d_rotation, float):
            rotations = pc.get_rotation.detach()
        else:
            # rotations = quaternion_multiply(pc.get_rotation, d_rotation).detach()
            rotations = quaternion_multiply(d_rotation , pc.get_rotation).detach()
    ...
    if colors_precomp is None:
        if pipe.convert_SHs_python:
            ...
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(pc.active_sh_degree, shs_view, dir_pp_normalized)
            colors_precomp = torch.clamp_min(sh2rgb + 0.5, 0.0)[valid].detach()
        else:
            shs = pc.get_features.detach()
    else:
