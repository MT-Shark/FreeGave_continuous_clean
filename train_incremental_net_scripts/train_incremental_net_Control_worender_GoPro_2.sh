#!/bin/bash

# 延迟 1.2 小时（1 小时 12 分钟）
# sleep 4320

densify_grad_threshold=0.0002
size_threshold=20
output=output
scenes="pen1 pen2 hammer box mat collision"
static_warmup_iter=10000
warmup_iter_basis=100
warmup_iter_gs=1500
iter_basis=100
iter_gs=1500
exp=net_woNodeWeight_zero_padding
# scenes=(pen1)
for scene in $scenes; do
    python incremental_train_ControlNode_worender_4.2.py \
        -s GVFi_datasets/${scene} \
        -m ${output}/GoPro/${scene}_controlNode_${exp}\
        --eval \
        --is_blender \
        --incremental_warmup_time 0 \
        --incremental_max_time 0.9 \
        --deform_max_time 0.05 \
        --iter_basis $iter_basis \
        --iter_gs $iter_gs \
        --noStatic_mask \
        --cn_init zero \
        --cn_KNN 1 \
        --cn_hyperdim 16 \
        --cn_KNN_method xyz \
        --control_num 10000 \
        --warmup_iter_basis $warmup_iter_basis \
        --warmup_iter_gs $warmup_iter_gs \
        --densify_grad_threshold ${densify_grad_threshold} \
        --size_threshold ${size_threshold} \
        --static_warmup_iter ${static_warmup_iter} \
        --zero_padding

    # python metrics_plot.py -m ${output}/${scene}_controlNode_${exp} --iter_basis 20 --iter_gs 100 --noStatic_mask
done



# densify_grad_threshold=0.0002
# size_threshold=20
# output=output
# scenes="pen1 pen2 hammer box mat collision"
# static_warmup_iter=10000
# warmup_iter_basis=120
# warmup_iter_gs=200
# iter_basis=120
# iter_gs=200
# exp=net_woNodeWeight_zero_padding_${static_warmup_iter}_${iter_basis}_${iter_gs}
# # scenes=(pen1)
# for scene in $scenes; do
#     python incremental_train_ControlNode_worender_4.2.py \
#         -s GVFi_datasets/${scene} \
#         -m ${output}/GoPro/${scene}_controlNode_${exp}\
#         --eval \
#         --is_blender \
#         --incremental_warmup_time 0.1 \
#         --incremental_max_time 0.9 \
#         --deform_max_time 0.05 \
#         --iter_basis $iter_gs \
#         --iter_gs $iter_gs \
#         --noStatic_mask \
#         --cn_init zero \
#         --cn_KNN 3 \
#         --cn_hyperdim 16 \
#         --cn_KNN_method xyz \
#         --control_num 10000 \
#         --warmup_iter_basis $warmup_iter_basis \
#         --warmup_iter_gs $warmup_iter_gs \
#         --densify_grad_threshold ${densify_grad_threshold} \
#         --size_threshold ${size_threshold} \
#         --static_warmup_iter ${static_warmup_iter} \
#         --zero_padding

#     # python metrics_plot.py -m ${output}/${scene}_controlNode_${exp} --iter_basis 20 --iter_gs 100 --noStatic_mask
# done